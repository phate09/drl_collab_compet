...
class ReplayBuffer(object):
    ...
    def sample(self, batch_size):
        ...
        samples = [self._storage[idx] for idx in idxes]
        return samples
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def add(self, data, tderror=None):
        idx = self._next_idx
        ...
        if self._next_idx >= len(self._storage):
            ...
            self._storage[self._next_idx] = data
        self._next_idx = (self._next_idx + 1) % self._maxsize
        tderror = self._max_priority if tderror is None else tderror + 1e-5  # adds a tiny amount to ensure it doesn't go to 0
        self._it_sum[idx] = tderror ** self._alpha
        self._it_min[idx] = tderror ** self._alpha
        self._max_priority = max(self._max_priority, tderror)
