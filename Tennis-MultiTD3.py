...
def main():
    ...
    config.train_every = 2
    config.train_n_times = 2
    config.n_step_td = 10
    config.ending_condition = ending_condition
    config.learn_start = config.max_t * 2  # training starts after this many transitions
    config.evaluate_every = 100
    config.use_noise = True
    config.use_priority = False
    config.noise_scheduler = Scheduler(1.0, 0.1, config.max_t * 1000, warmup_steps=config.max_t * 2)
    ...
    for i_episode in range(config.n_episodes):
        ...
        score = 0
        noise_magnitude=config.noise_scheduler.get(global_steps)
        for i in range(config.max_t):
            with torch.no_grad():
                actions = agent.act(states, config.noise_scheduler.get(global_steps))  # todo introduce noise magnitude
